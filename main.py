...
def create_server(mc_version, latest):
    try:
        ...
        url = 'https://papermc.io/api/v1/paper/{MCVERSION}/{latest}/download'.format(MCVERSION=mc_version, latest=latest)
...
def update_server(mc_version, latest):
    try:
        ...
        shutil.move('./temp/paper.jar', './paper.jar')
        print('Cleaning up...')
...
def check_version():
    ...
    paper_ver_req = requests.get('https://papermc.io/api/v1/paper/{MCVERSION}/'.format(MCVERSION=paper_mc_version))
    ...
    if check_for_install():
        ...
        create_server(paper_mc_version, api_paper_ver)

    if int(current_paper_ver[0]) == int(api_paper_ver):
        print('Up To Date!')
    else:
        print('Updating!')
        update_server(paper_mc_version, api_paper_ver)
