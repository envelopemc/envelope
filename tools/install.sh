...
main() {
    ...
    printf "${GREEN}Checking python install...${NORMAL}\n"
    ...
    printf "${BLUE}Cloning Envelope${NORMAL}\n"
    ...
    printf "${GREEN}Creating Python virtual-enviroment${NORMAL}\n"
    cd envelope/ || {
      exit 1
    }
    python3 -m venv venv || {
    ...
    printf "${GREEN}Installing required packages..${NORMAL}\n"
    source venv/bin/activate
    pip install -r requirements.txt || {
        printf "installation of python packages failed...\n"
    }

    printf "${BLUE}${BOLD}Envelope successfully installed!${NORMAL}\n"
}
